...
function tok(u){ return Buffer.from(u).toString('base64'); }
...
const server=http.createServer(async(req,res)=>{
  ...
  if(req.method==='POST'&&path==='/login'){
    ...
    // 🔴 CF-3: weak token, no HttpOnly, no Secure
    if(isApi()){
...
server.listen(PORT,()=>{
  ...
  console.log(`  BAC-2  FakeToken   →  cookie: token=${Buffer.from('admin').toString('base64')}`);
